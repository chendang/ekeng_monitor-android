//(模拟ajax接口)测试数据,需要删除(还有一部分在index.js开头)
...
+function ($) {
    ...
    var Calendar = function (params) {
        ...
        var defaults = {
            ...
            firstDay: 7, // First day of the week, Monday
        ...
        p.initCalendarEvents = function () {
            ...
            function handleDayClick(e) {
                ...
                var dataDay = dateDay;
                //ajax 获得监测事件
                var dataData = $(this).attr('data-date');
                if(parseInt(dataDay) < 10){
                    dataDay = "0"+dataDay;
                }
                $(".databg-showdata").text(dataDay);
                var yearMonth = dataYear+"年"+dataMonth+"月";
                $(".databg-showinfo-yearmonth").text(yearMonth);

                var date = dataMonth+"/"+dataDay+"/"+dataYear;
                var day = new Date(Date.parse(date));
                var week = today[day.getDay()];
                $(".databg-showinfo-week").text(week);
                showDateText(dataYear,dataMonth,dataDay);
                var daycode=dataYear+dataMonth+dataDay

                getDayMonitorSet(username,daycode);
        ...
        p.monthHTML = function (date, offset) {
            ...
            var dayDate, currentValues = [], i, j,
                rows = 6, cols = 7,
                monthHTML = '',
                dayIndex = 0 + (p.params.firstDay - 7),
            ...
            for (i = 1; i <= rows; i++) {
                ...
                for (j = 1; j <= cols; j++) {
                    ...
                    rowHTML += '<div data-year="' + dayYear + '" data-month="' + dayMonth + '" data-day="' + dayNumber + '" class="picker-calendar-day day-select' + (addClass) + '" data-date="' + (dayYear + '-' + dayMonth + '-' + dayNumber) + '"><span>'+dayNumber+'</span></div>';
                }
        ...
        p.updateCurrentMonthYear = function (dir) {
            ...
            var mycurrentMonth = parseInt(p.currentMonth)+1;
            var mycurrentYear = p.currentYear;
            if(mycurrentMonth < 10){
                mycurrentMonth = "0"+mycurrentMonth;
            }
            var myyearMonth = mycurrentYear+"年"+mycurrentMonth+"月";
            $(".databg-showinfo-yearmonth").text(myyearMonth);
        };
